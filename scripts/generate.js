...
let generate =
{
    ...
    expr: function(steps, variables) {
        ...
        if (fn === "coeff") {
            return new exprCstr.coeff(helper.randint(settings.min, settings.max),
                                      generate.expr(steps - 1, variables));
        } else if (fn === "sum") {
            // have chance of one being a constant?
            return new exprCstr.sum(generate.expr(steps - 1, variables),
                                    generate.expr(steps - 1, variables));
        } else if (fn === "product") {
            return new exprCstr.product(generate.expr(steps - 1, variables),
                                        generate.expr(steps - 1, variables));
        } else if (fn === "quotient") {
            // have chance of one being a constant?
            return new exprCstr.quotient(generate.expr(steps - 1, variables),
                                         generate.expr(steps - 1, variables));
        } else if (fn === "power") {
            ...
            return new exprCstr.power(generate.expr(steps - 1, variables),
                                      power);
        } else if (fn === "etothe") {
            return new exprCstr.etothe(generate.expr(steps - 1, variables));
        } else if (fn === "trig") {
            let tfn = trigFns[Math.floor(Math.random() * trigFns.length)];
            console.log(tfn);
            return new exprCstr[tfn](generate.expr(steps - 1, variables));
        }
    }
}